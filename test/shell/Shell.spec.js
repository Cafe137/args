...
it('should generate fish completion', () => {
    ...
    expect(script).toBe(`function _cafe_cli_completion
    cafe-cli --compgen (commandline -pb)
end
complete -f -c cafe-cli -a '(_cafe_cli_completion)'`)
})

it('should generate zsh completion', () => {
    const script = generateCompletion('cafe-cli', 'zsh')
    expect(script).toBe(`_cafe_cli_complete() {
    compadd -- \`cafe-cli --compgen "\${BUFFER}"\`
}
compdef _cafe_cli_complete cafe-cli`)
})

it('should generate bash completion', () => {
    const script = generateCompletion('cafe-cli', 'bash')
    expect(script).toBe(`_cafe_cli_completion() {
    local cur prev nb_colon
    _get_comp_words_by_ref -n : cur prev
    nb_colon=$(grep -o ":" <<< "$COMP_LINE" | wc -l)
    COMPREPLY=( $(compgen -W '$(cafe-cli --compgen "\${COMP_LINE}")' -- "$cur") )
    __ltrim_colon_completions "$cur"
}
complete -F _cafe_cli_completion cafe-cli`)
})
